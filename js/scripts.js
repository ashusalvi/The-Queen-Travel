...
(function ($) {
    ...
    $("#placeContactForm").validator().on("submit", function (event) {
        if (event.isDefaultPrevented()) {
            // handle the invalid form...
            cformError();
            csubmitMSG(false, "Please fill all fields!");
    ...
    function placecsubmitForm() {
        ...
        $.ajax({
            type: "POST",
            url: "php/place-contactform-process.php",
            data: "name=" + name + "&email=" + email + "&message=" + message + "&terms=" + terms + "&no_of_pax=" + no_of_pax + "&mobile=" + mobile + "&package_name=" + package_name,
            success: function (text) {
                if (text == "success") {
                    cformSuccess();
                } else {
